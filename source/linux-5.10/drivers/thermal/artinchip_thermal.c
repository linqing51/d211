 ...
 */
static void aic_tsen_curve_fitting(int ch_id, int y1, int y2, int x1, int x2,
...
static int aic_tsen_nvmem_read(struct aic_tsen_dev *tsen)
{
	struct device *dev = &tsen->pdev->dev;
	struct aic_tsen_ch_dat *dat = tsen->data->ch;
	...
	for (i = 0; i < TSEN_NVMEM_CELL_NUM ; i++) {
		...
		buf[i] = *(u16 *)nvmem_cell_read(cell, &len);
		if (buf[i] == 0) {
	...
	aic_tsen_curve_fitting(AIC_TSEN_CH_CPU, buf[TSEN_THS0_ADC_VAL_LOW],
			       buf[TSEN_THS0_ADC_VAL_HIGH], env_temp_low,
			       env_temp_high, dat);
	aic_tsen_curve_fitting(AIC_TSEN_CH_GPAI, buf[TSEN_THS1_ADC_VAL_LOW],
			       buf[TSEN_THS0_ADC_VAL_HIGH], env_temp_low,
			       env_temp_high, dat);
	return 0;
}
