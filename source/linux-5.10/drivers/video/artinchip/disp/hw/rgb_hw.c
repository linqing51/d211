...
u32 i8080_rd_fifo_depth(void __iomem *base)
{
	return reg_rd_bits(base + I8080_FIFO_DEPTH,
			RGB_I8080_RD_FIFO_DEPTH_MASK,
			RGB_I8080_RD_FIFO_DEPTH_SHIFT);
}

u32 i8080_wr_fifo_depth(void __iomem *base)
{
	return reg_rd_bits(base + I8080_FIFO_DEPTH,
			RGB_I8080_WR_FIFO_DEPTH_MASK,
			RGB_I8080_WR_FIFO_DEPTH_SHIFT);
}

void i8080_rd_fifo_flush(void __iomem *base)
{
	reg_set_bit(base + I8080_FIFO_DEPTH, RGB_I8080_RD_FIFO_FLUSH);
}
...
void i8080_cmd_wr(void __iomem *base, u32 code, u32 count, const u8 *data)
{
	...
	while (!reg_rd_bit(base + I8080_STATUS, RGB_I8080_TX_FIFO_EMPTY,
		RGB_I8080_TX_FIFO_EMPTY_SHIFT))
		;
}
...
u32 spi_rd_fifo_depth(void __iomem *base)
{
	return reg_rd_bits(base + SPI_FIFO_DEPTH,
			RGB_SPI_RD_FIFO_DEPTH_MASK,
			RGB_SPI_RD_FIFO_DEPTH_SHIFT);
}

u32 spi_wr_fifo_depth(void __iomem *base)
{
	return reg_rd_bits(base + SPI_FIFO_DEPTH,
			RGB_SPI_WR_FIFO_DEPTH_MASK,
			RGB_SPI_WR_FIFO_DEPTH_SHIFT);
}

void spi_rd_fifo_flush(void __iomem *base)
{
	reg_set_bit(base + SPI_FIFO_DEPTH, RGB_SPI_RD_FIFO_FLUSH);
}

void spi_wr_fifo_flush(void __iomem *base)
{
	reg_set_bit(base + SPI_FIFO_DEPTH, RGB_SPI_WR_FIFO_FLUSH);
}
...
void spi_cmd_wr(void __iomem *base, u32 code, u32 count, const u8 *data)
{
	...
	while (!reg_rd_bit(base + SPI_STATUS, RGB_SPI_TX_FIFO_EMPTY,
		RGB_SPI_TX_FIFO_EMPTY_SHIFT))
		;
}
