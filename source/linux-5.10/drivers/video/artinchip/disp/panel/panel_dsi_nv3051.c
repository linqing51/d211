...
static inline struct nv3051f *panel_to_nv3051f(struct aic_panel *panel)
{
	return (struct nv3051f *)panel->panel_private;
}

static int panel_enable(struct aic_panel *panel)
{
	struct nv3051f *nv3051f = panel_to_nv3051f(panel);
	int ret;

	gpiod_direction_output(nv3051f->reset, 1);
	aic_delay_ms(120);
	gpiod_direction_output(nv3051f->reset, 0);
	aic_delay_ms(120);
	gpiod_direction_output(nv3051f->reset, 1);
	aic_delay_ms(120);
...
{
	struct panel_comp *p;
	struct nv3051f *nv3051f;

	p = devm_kzalloc(dev, sizeof(*p), GFP_KERNEL);
	if (!p)
		return -ENOMEM;

	nv3051f = devm_kzalloc(dev, sizeof(*nv3051f), GFP_KERNEL);
	if (!nv3051f)
		return -ENOMEM;

	nv3051f->reset = devm_gpiod_get(dev, "reset", GPIOD_ASIS);
	if (IS_ERR(nv3051f->reset)) {
		dev_err(dev, "failed to get reset gpio\n");
		return PTR_ERR(nv3051f->reset);
	}
	...
	p->panel.dsi = &dsi;
	panel_init(p, dev, &panel_vm, &panel_funcs, nv3051f);
