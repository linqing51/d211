		...
		OMX_IN	OMX_BUFFERHEADERTYPE* pBuffer)
{
	...
	if(pVdecDataType->sInPortTunneledInfo.nTunneledFlag){
		if(pVdecDataType->sInBufSupplier.eBufferSupplier == OMX_BufferSupplyOutput){
			...
			mpp_list_add_tail(&pktNode->sList, &pVdecDataType->sInReadyPkt);
			aic_pthread_mutex_unlock(&pVdecDataType->sInPktLock);
			...
			logi("pVdecDataType->nReceivePacktOkNum:%d\n",pVdecDataType->nReceivePacktOkNum);
	...
	while(1){
		...
		if((pVdecDataType->sInPortTunneledInfo.nTunneledFlag) ){
			if(!OMX_VdecListEmpty(&pVdecDataType->sInReadyPkt,pVdecDataType->sInPktLock)){
				...
				struct mpp_packet pkt;
				while(!OMX_VdecListEmpty(&pVdecDataType->sInReadyPkt,pVdecDataType->sInPktLock)){
					aic_pthread_mutex_lock(&pVdecDataType->sInPktLock);
					pktNode1 = mpp_list_first_entry(&pVdecDataType->sInReadyPkt, VDEC_IN_PACKET, sList);
					aic_pthread_mutex_unlock(&pVdecDataType->sInPktLock);
					...
					ret = mpp_decoder_put_packet(pVdecDataType->pDecoder, &pkt);
					if(pVdecDataType->nPutPacktToDecoderOkNum < OMX_VDEC_PRINT_FRAME_NUM){
						logd(" ret:%d,pkt.pts:%lld,nPutPacktToDecoderOkNum:%d\n"
						,ret,pkt.pts,pVdecDataType->nPutPacktToDecoderOkNum);
					}

					pVdecDataType->nPutPacktToDecoderOkNum++;
					logd("pVdecDataType->nPutPacktToDecoderOkNum:%d\n",pVdecDataType->nPutPacktToDecoderOkNum);

					if(pkt.flag & PACKET_FLAG_EOS){
						pVdecDataType->nFlags |= VDEC_INPORT_STREAM_END_FLAG;
						printf("[%s:%d] send nStreamEndFlag\n",__FUNCTION__,__LINE__);
					}
					...
					*/
					...
					aic_msg_wait_new_msg(&pVdecDataType->sMsgQue, 10*1000);
			}
		}
		...
		if((pVdecDataType->sInPortTunneledInfo.nTunneledFlag)
			...
			while(!OMX_VdecListEmpty(&pVdecDataType->sInProcessedPkt,pVdecDataType->sInPktLock)){
				...
				if(ret == 0){
					...
					pVdecDataType->nGiveBackPacktFailNum++;
					loge("pAdecDataType->nGiveBackPacktOkNum:%d,pAdecDataType->nGiveBackPacktFailNum:%d\n"
						,pVdecDataType->nGiveBackPacktOkNum
						,pVdecDataType->nGiveBackPacktFailNum);
					break;
				}
		...
		// decode
		if(!(pVdecDataType->nFlags & VDEC_DECODER_CONSUME_ALL_INPORT_STREAM_FLAG)){//!(pVdecDataType->nFlags & VDEC_DECODER_CONSUME_ALL_INPORT_STREAM_FLAG)
			ret = mpp_decoder_decode(pVdecDataType->pDecoder);
			if(ret == DEC_OK){
				logi("mpp_decoder_decode ok!!!\n");
			}else if(ret == DEC_NO_READY_PACKET){
			logi("mpp_decoder_decode error DEC_NO_READY_PACKET !!!\n");
				if(pVdecDataType->nFlags & VDEC_INPORT_STREAM_END_FLAG){//(pVdecDataType->nFlags & VDEC_INPORT_STREAM_END_FLAG)
					pVdecDataType->nFlags |= VDEC_DECODER_CONSUME_ALL_INPORT_STREAM_FLAG;//pVdecDataType->nFlags |= VDEC_DECODER_CONSUME_ALL_INPORT_STREAM_FLAG
					logd("mpp_decoder_decode notify stream end !!!\n");
					// stream end  trans to idle ????
					if(pVdecDataType->pCallbacks && pVdecDataType->pCallbacks->EventHandler) {
							pVdecDataType->pCallbacks->EventHandler(pVdecDataType->hSelf,
												pVdecDataType->pAppData,OMX_EventBufferFlag,0, 0,NULL);
					}
				}else{
					// wait some time
					usleep(1000);
		...
		if(!(pVdecDataType->nFlags & VDEC_GET_ALL_FRAME_FREOM_DECODER_FLAG)){//!(pVdecDataType->nFlags & VDEC_GET_ALL_FRAME_FREOM_DECODER_FLAG)
			if(!OMX_VdecListEmpty(&pVdecDataType->sOutEmptyFrame,pVdecDataType->sOutFrameLock)){
				...
				if(ret == DEC_OK){
					...
					mpp_list_del(&pFrameNode->sList);
					mpp_list_add_tail(&pFrameNode->sList, &pVdecDataType->sOutReadyFrame);
					aic_pthread_mutex_unlock(&pVdecDataType->sOutFrameLock);
					if(pVdecDataType->nGetFrameFromDecoderNum < OMX_VDEC_PRINT_FRAME_NUM){
						logd("nGetFrameFromDecoderNum:%u,pts:%lld",pVdecDataType->nGetFrameFromDecoderNum,pFrameNode->sFrameInfo.pts);
					}
					pVdecDataType->nGetFrameFromDecoderNum++;
					logi("pVdecDataType->nGetFrameFromDecoderNum:%d\n",pVdecDataType->nGetFrameFromDecoderNum);
		...
		if(!OMX_VdecListEmpty(&pVdecDataType->sOutReadyFrame,pVdecDataType->sOutFrameLock)){
			...
			while(!OMX_VdecListEmpty(&pVdecDataType->sOutReadyFrame,pVdecDataType->sOutFrameLock)){
				aic_pthread_mutex_lock(&pVdecDataType->sOutFrameLock);
				pFrameNode = mpp_list_first_entry(&pVdecDataType->sOutReadyFrame, VDEC_OUT_FRAME, sList);
				aic_pthread_mutex_unlock(&pVdecDataType->sOutFrameLock);
				...
				if(pVdecDataType->sOutPortTunneledInfo.nTunneledFlag){
					...
					ret = OMX_EmptyThisBuffer(pVdecDataType->sOutPortTunneledInfo.pTunneledComp,&sBuffHead);
					...
					if(pVdecDataType->pCallbacks != NULL && pVdecDataType->pCallbacks->FillBufferDone != NULL){
						ret = pVdecDataType->pCallbacks->FillBufferDone(pVdecDataType->hSelf,pVdecDataType->pAppData,&sBuffHead);
				...
				if(ret == 0){
					...
					if(pFrameNode->sFrameInfo.flags & FRAME_FLAG_EOS){
						pVdecDataType->nFlags |= VDEC_GET_ALL_FRAME_FREOM_DECODER_FLAG;
						logd("nFrameEndFlag");
					...
					if(pVdecDataType->pCallbacks && pVdecDataType->pCallbacks->EventHandler) {
							pVdecDataType->pCallbacks->EventHandler(pVdecDataType->hSelf,
									pVdecDataType->pAppData,OMX_EventError,
									OMX_ErrorUndefined, pVdecDataType->state,NULL);
					}
					logw("OMX_EmptyThisBuffer or FillBufferDone fail!\n");
					//ret = mpp_decoder_put_frame(pAdecDataType->pDecoder, &pFrameNode->sFrameInfo);
					//if(ret != 0){// how to do
					//	loge("mpp_decoder_put_frame error!!!!\n");
					//}
					// now no matter whether mpp_decoder_put_frame is ok, I think give fame back ok.
					//mpp_list_del(&pFrameNode->sList);
			...
			if(pVdecDataType->nFlags & VDEC_GET_ALL_FRAME_FREOM_DECODER_FLAG){
				pVdecDataType->nFlags |= VDEC_OUTPORT_SEND_ALL_FRAME_FLAG;
				logi("VDEC_OUTPORT_SEND_ALL_FRAME_FLAG\n");
			}
