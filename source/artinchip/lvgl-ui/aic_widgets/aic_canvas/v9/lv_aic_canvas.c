...
lv_res_t lv_aic_canvas_alloc_buffer(lv_obj_t *obj, lv_coord_t w, lv_coord_t h)
{
    ...
    uint32_t stride = canvas->img_buf->dec_buf.stride[0];
    void *data = (void *)canvas->img_buf->data[0];
    lv_draw_buf_init(&canvas->img_buf->decoded, w, h, cf, stride, data, stride * h);
    canvas->draw_buf = &canvas->img_buf->decoded;

    // means aic mpp buffer
    canvas->draw_buf->header.flags |= LV_IMAGE_FLAGS_USER8;
...
void lv_aic_canvas_draw_text_to_center(lv_obj_t *obj, lv_draw_label_dsc_t *label_dsc, const char *txt)
{
    ...
    if (canvas->img_buf) {
        lv_ge_fill(&canvas->img_buf->dec_buf, GE_NO_GRADIENT, 0x00000000, 0x00000000, 0);
