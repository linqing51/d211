...
struct aic_player_ctx_s {
    ...
    uint8_t * image_data;
    struct player_screen_info screen_info;
...
static void obtain_screen_info(struct aic_player_ctx_s *aic_ctx);
...
static int lv_player_create_image_resource(lv_obj_t * obj);
static int lv_player_get_image_area(lv_obj_t * obj, lv_area_t *area);
static int lv_player_set_image_src(lv_obj_t * obj);
static int lv_player_update_video_area(lv_obj_t * obj);
...
lv_res_t lv_aic_player_set_src(lv_obj_t * obj, const char *src)
{
    ...
    if (!player->aic_ctx) {
        ...
        obtain_screen_info(player->aic_ctx);
    } else {
    ...
    if (aic_ctx->media_info.has_audio == 1 && aic_ctx->media_info.has_video == 0) {
        ...
        if (player->draw_layer == LV_AIC_PLAYER_LAYER_DEFAULT) {
            ...
            aic_ctx->draw_layer = LV_AIC_PLAYER_LAYER_VIDEO;
        }
    ...
    if ((aic_ctx->draw_layer == LV_AIC_PLAYER_LAYER_UI_SINGLE_BUF ||
        aic_ctx->draw_layer == LV_AIC_PLAYER_LAYER_UI_DOUBLE_BUF)) {
        res = lv_player_create_image_resource(obj);
        if (res < 0) {
            LV_LOG_ERROR("lv_player_create_image_resource failed");
            goto src_failed;
        }
    }
    ...
    if (aic_ctx->draw_layer == LV_AIC_PLAYER_LAYER_VIDEO) {
        ...
        lv_player_set_image_src(obj);
        lv_player_update_video_area(obj);
...
static int player_get_frame_sync(struct aic_player_ctx_s * aic_ctx, bool sleep_en)
{
    ...
    do {
        /* return 0 when reaching the appropriate video frame time */
...
static void obtain_screen_info(struct aic_player_ctx_s *aic_ctx)
{
    ...
    memcpy(&aic_ctx->screen_info, &screen_info, sizeof(struct aicfb_screeninfo)); 
#else
    ...
    aic_ctx->screen_info.width = var.xres;
    aic_ctx->screen_info.height = var.yres;
    aic_ctx->screen_info.smem_len = fix.smem_len;
    aic_ctx->screen_info.stride = fix.line_length;
#endif
}
...
#endif
    if (!(rotate == 900 || rotate == 1800 || rotate == 2700)) {
        return true;
    }
...
static int lv_player_get_image_area(lv_obj_t * obj, lv_area_t *area)
{
    ...
    lv_area_t tmp_area = {obj->coords.x1, obj->coords.y1,
                      obj->coords.x1 + image->w - 1, obj->coords.y1 + image->h - 1};
    if(image->align < _LV_IMAGE_ALIGN_AUTO_TRANSFORM) {
        lv_area_align(&obj->coords, &tmp_area, image->align, image->offset.x, image->offset.y);
    ...
    lv_memcpy(area, &tmp_area, sizeof(lv_area_t));
    _lv_image_buf_get_transformed_area(area, lv_area_get_width(area), lv_area_get_height(area),
                                       image->rotation, image->scale_x,
                                       image->scale_y, &pivot_px);
    lv_area_move(area, tmp_area.x1, tmp_area.y1);
...
static int lv_player_set_image_src(lv_obj_t * obj)
{
    ...
    if ((lv_player_get_image_area(obj, &area)) == -1)
        return -1;
    ...
    int alpha_en = 0;
    int image_width = ((int)area.x2 - (int)area.x1) + 1;
    int image_height = ((int)area.y2 - (int)area.y1) + 1;
...
static int lv_player_update_video_area(lv_obj_t *obj)
{
    ...
    disp_rect.x = area.x1;
    disp_rect.y = area.y1;
    disp_rect.width = area.x2 - area.x1 + 1;
    disp_rect.height = area.y2 - area.y1 + 1;
