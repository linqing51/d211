...
check_work_path()
{
	...
	# 2. The current path is Luban/scripts/tools/
	if [ -d source ]; then
		GCC_PKG_DIR=$TOPDIR/scripts/tools/gcc
	else
		GCC_PKG_DIR=$TOPDIR/gcc
	fi
}
...
compile_install_gcc()
{
	...
	if [ $INTERNET_IS_AVAILABLE -eq $TRUE ]; then
		...
		MPFR=mpfr-2.4.2
		tar xjf ../$MPFR.tar.bz2 || exit $ERR_PKG_UNVAILABLE
		ln -sf $MPFR mpfr || exit $ERR_PKG_UNVAILABLE

		GMP=gmp-4.3.2
		tar xjf ../$GMP.tar.bz2  || exit $ERR_PKG_UNVAILABLE
		ln -sf $GMP gmp || exit $ERR_PKG_UNVAILABLE

		MPC=mpc-0.8.1
		tar xzf ../$MPC.tar.gz || exit $ERR_PKG_UNVAILABLE
		ln -sf $MPC mpc || exit $ERR_PKG_UNVAILABLE

		ISL=isl-0.15
		tar xjf ../$ISL.tar.bz2 || exit $ERR_PKG_UNVAILABLE
