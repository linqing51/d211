...
function ckernel()
{
	...
	kernel_dir="${LUBAN_PRJ_TOP_DIR}/source/linux-5.10"
	cd ${kernel_dir}
}
alias ck=ckernel
...
function cuboot()
{
	...
	uboot_dir="${LUBAN_PRJ_TOP_DIR}/source/uboot-2021.10"
	cd ${uboot_dir}
}
alias cu=cuboot
...
function genindex()
{
	...
	case ${result} in
		"linux")
			gen_path=${LUBAN_PRJ_TOP_DIR}/source/linux-5.10
		;;
		"uboot")
			gen_path=${LUBAN_PRJ_TOP_DIR}/source/uboot-2021.10
		;;
		"third-party")
...
function _get_dir_list()
{
	...
	dir_list1+="${sep}"
	dir_list1+=`find ${LUBAN_PRJ_TOP_DIR}/scripts/ -type d ! -path "*/.*"`
	dir_list1+="${sep}"
...
function _mark_topdir()
{
	...
	if [ -f scripts/envsetup.sh ]; then
		LUBAN_PRJ_TOP_DIR=$(pwd)
	elif [ -f ../scripts/envsetup.sh ]; then
		LUBAN_PRJ_TOP_DIR=$(cd .. && pwd)
	else
		echo 'Please "source scripts/envsetup.sh" in Luban SDK Root directory'
		return
	fi
	if [ -f ${LUBAN_PRJ_TOP_DIR}/scripts/tools/bin/fff ]; then
		LUBAN_PRJ_FFF=${LUBAN_PRJ_TOP_DIR}/scripts/tools/bin/fff
	fi
}
...
export PATH=$PATH:${LUBAN_PRJ_TOP_DIR}/scripts/tools/bin
