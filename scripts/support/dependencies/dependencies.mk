...
define suitable-host-package
$(if $(1),$(shell scripts/support/dependencies/check-host-$(1).sh $(2)))
endef
endif
# host utilities needs host-tar to extract the source code tarballs, so
# ensure check-host-tar.mk is included before the rest
include scripts/support/dependencies/check-host-tar.mk
-include $(sort $(filter-out %-tar.mk,$(wildcard scripts/support/dependencies/check-host-*.mk)))

dependencies:
	@MAKE="$(MAKE)" DL_TOOLS="$(sort $(DL_TOOLS_DEPENDENCIES))" \
		$(TOPDIR)/scripts/support/dependencies/dependencies.sh
